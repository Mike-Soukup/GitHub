...
hist(normal_x, breaks = cells, col = "blue")
curve(dnorm(x,mean=0,sd=1),add=TRUE, col="black",lwd=2)
...
max(mag)
mag.2 <- c(0.70, 0.74, 0.64, 0.39, 0.70, 2.20, 1.98, 0.64, 1.22, 0.20, 1.64, 1.02, 
           2.95, 0.90, 1.76, 1.00, 1.05, 0.10, 5.0, 1.56, 1.62, 1.83, 0.99, 1.56,
           0.40, 1.28, 0.83, 1.24, 0.54, 1.44, 0.92, 1.00, 0.79, 0.79, 1.54, 1.00,
           2.24, 2.50, 1.79, 1.25, 1.49, 0.84, 1.42, 1.00, 1.25, 1.42, 1.15, 0.93,
           0.40, 1.39)
cells.2 <- seq(from = 0.0, to = 5.5, by = 0.5)
hist(mag.2, breaks = cells.2, col = "red", right = FALSE,
     main = "Histogram of Mag.2", xlab = "Mag.2")

qqnorm(mag.2, col = "red", pch = 16)
qqline(mag.2, col = "green", lty = 2, lwd = 2)
