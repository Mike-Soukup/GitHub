library(ggplot2)
df <- read.csv("/Users/mikeysoukup/Desktop/Python/F1/Belgian GP 2020.csv")
...
ggplot(data = df.race, aes(x = X)) +
  ...
  labs(title = "2020 Belgian Grand Prix", x = "Lap", y = "Position") +
  theme(axis.text.x = element_text(angle = 90, size = 10),
        plot.title = element_text(hjust = 0.5)) 
################################################################################
Constructors_after_teams <- c("Mercedes","Red Bull Racing Honda",
                              "McLaren Renault","Racing Point BWT Mercedes",
                              "Ferrari","Ranault","AlphaTauri Honda",
                              "Alfa Romeo Racing Ferrari", "Haas Ferrari",
                              "Williams Mercedes")
Constructors_after_points <- c(264,158,68,66,61,59,20,2,1,0)
Constructors_before_points <- c(221,135,62,63,61,36,16,2,1,0)
Constructors_change <- Constructors_after_points - Constructors_before_points

Drivers_after_names <- c("Lewis Hamilton","Max Verstappen","Valtteri Bottas",
                         "Alexander Albon","Charles Leclerc","Lando Norris",
                         "Lance Stroll","Daniel Ricciardo","Sergio Perez",
                         "Esteban Ocon")
Drivers_after_points <- c(157,110,107,48,45,45,42,33,33,26)
Drivers_before_points <- c(132,95,89,40,45,39,40,20,32,16)
Drivers_change <- Drivers_after_points - Drivers_before_points

C.mat <- matrix(nrow = 10, ncol = 4, byrow = FALSE)
C.mat[,1] <- as.character(Constructors_after_teams) 
C.mat[,2] <- as.numeric(Constructors_before_points)
C.mat[,3] <- as.numeric(Constructors_after_points)
C.mat[,4] <- as.numeric(Constructors_change)
colnames(C.mat) <- c("Constructor","Before Spa","After Spa","Delta")
noquote(C.mat)

D.mat <- matrix(nrow = 10, ncol = 4, byrow = FALSE)
D.mat[,1] <- Drivers_after_names
D.mat[,2] <- Drivers_before_points
D.mat[,3] <- Drivers_after_points
D.mat[,4] <- Drivers_change
colnames(D.mat) <- c("Driver","Before Spa","After Spa","Delta")
noquote(D.mat)